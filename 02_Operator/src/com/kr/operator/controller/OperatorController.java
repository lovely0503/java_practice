package com.kr.operator.controller;
...
public class OperatorController {
	...
	public void arithmetic() {
		...
		//*,/,%->우선순위가 +,-보다 높기때문에 묶어주지않더라도 먼저 연산을 수행
		//다른 개발자들의 가독성을 위해 괄호를 사용하는것을 권장
		System.out.println("num1 / num2 :"+(num1/num2));
	...
	public void test() {
		System.out.println("메소드를 호출할때 꼭 메인에서만 불러야하는것은 아닙니다");
	}
	//사탕개수 구하기 프로그램
	...
	//1인당 받을수있는 사탕의 개수 : 사탕개수 / 학생수 <== 산술연산자
	// 남은 사탕의 개수 : 사탕의 개수 % 학생수  <= 산술연산자
	
	//출력형식
	//학생 한명당 받을수 있는 사탕개수 : x개  <==println();
	//남은 사탕 개수 :x개 <== println();
	...
	public void presentToStudent() {
		...
		//System.out.println("사탕개수 :" + candies);
		...
		System.out.println("두당 가질수 없는 사탕의 개수 : "+result + "개");
		System.out.println("남는 사탕의 개수 : "+mod+"개");
	}
	
	public void inDecrease() {
		/*
		 * 증감 연산자 : 단항연산자로 한번에 1증가하거나 /1 감소하는 연산을함
		 * [표현법]
		 * ++ : 값을 1증가 시킴
		 * -- : 값을 1 감소 시킴
		 * 
		 ...
		 * 
		 * 전위 연산 : ++값, --값		--> 먼저 값을 증가 또는 감소 시키고 나서 작업을 처리
	 ...
	 public void compound() {
		 //= 대입연산자
		 // 산술연산자에 대입연산자를 합친 연산자 : 복합 대입연산자
		 // 자기자신과 해당 산술연산을 수행한후 그 결과를 자기자신에게 다시 대입하는 용도
		 // +=
		 // -=
		 // *=
		 // /=
		 ...
		 endDay-=1;
		 System.out.println(endDay);
		 long veryBigNumberCount = 1000000L;
	 ...
	 public void comparsion() {
		  ...
		  *관계 연산자(비교연산자)
		  * 
		  *두 개의 값을 가지고 비교하는, 이항연산자
		  *비교연산을 한 결과 -> true,false
		  *특정 조건을 제시할수 있는 조건문에서 조건식으로 사용할것 
		  */
		 /*
		  * 종류
		  * 
	 ...
	 public void logical() {
		  ...
		  * 
		  * 1. AND 연산 : 논리값 && 논리값
		  * 좌항과 우항의 값이 모두 true이어야만 최종 결과값이 true 
		  * 
		  * 2. OR 연산 : 논리값 || 논리값
		  * 좌항과 우항의 값이 하나라도 true일 경우 최종 결과값이 true
		  */
		 //계획
		 //1. 사용자에게 한개의 정수값을 입력받음
	 ...
	 public void orOper() {
		 ...
		 System.out.print("꼭 한글자만 입력해주세요 > ");
		 ...
		 System.out.println("사용자가 입력한값이 에이 인가요? >" + result);
	 }
	  ...
	  * 
	  * 조건문 : 값에따라 연산을 처리하는 방식
	  * 	결과값이 true일 경우 첫번째 문장을 처리
	  * 	결과값이 false일 경우 두번째 문장을 쳐리
	  * 	
	  */
	 ...
	 public void triple() {
		 ...
		 //메뉴번호가 1번일시 AI치킨이 나오게하고 1번이 아닐시 없는 메뉴입니다 라고 출력
