...
public class FiledView {
	...
	public void check() {
		...
		//2.객체생성(new키워드를 만났을때) -> global 멤버변수 할당
		//3.생성된 객체의 주소값을 대입
		 ...
		 * 
		 * public => 어디서든(같은패키지, 다른패키지, 클래스 내/외부 포함 전부) 접근 가능
		 * 
		 * protected => 같은 패키지라면 무조건 접근가능!
		 * 			 => 다른 패키지라면 상속구조인 클래스에서만 접근가능
		 * 			 => 상속 07번 프로젝트에서 배울것
		 * 
		 * 
		 * default(package friendly) => 나랑 같은 패키지에서만 가능
		 * 
