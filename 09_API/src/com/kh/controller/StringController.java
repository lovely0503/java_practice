...
public class StringController {
	...
	public void method1() {
		...
		//1.String 클래스의 toString()의경우
		//실제담겨있는 문자열 value값을 반환하게끔 오버라이딩
		...
		//3.주소값을 해싱하는것이 아니라 실제담긴 문자열값을 기반으로 해싱을 돌리도록 오버라이딩
	...
	public void method3() {
		...
		//문자열 리터럴은 대입연산자를 사용해 대입할때
		//String Pool(heap) 영역에 등록됨
		//StringPool : 동일한 내용의 문자열 value다 존재할수없음
	...
	public void method4() {
		...
		//문자열 임시저장공간
		//문자열이 불변객체라서 내용물 변경이 일어나면 계속 새로운 공간을 할당해서 올라감
		//이를 해결하기위해서 임시공간(Buffer)를 준비해서 임시공간에 차곡차곡 담았다가 한번에 처리하는 클래스
	...
	public void method() {
		...
		String userId = "sdfsdsfrdfs";
		//아이디 8자~20자
		if(!(8 <= userId.length() && userId.length() <= 20)) {
			throw new RuntimeException("아이디 글자수 어쩌고");
		}
		//2.문자열.charAt() : char
		...
		String fileName2 = "sd.sdf.sdf.sdf.sdf.png";
